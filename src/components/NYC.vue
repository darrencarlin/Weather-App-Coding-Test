<template>
<div class='main' v-if='nyc' v-bind:class="{ 'day': day, 'evening': evening, 'night': night }">
	<div class='controls' >
		 <router-link to='/'><i class='fas fa-arrow-circle-left fa-2x'></i></router-link>
		<p class='title'>{{nyc.name}}</p>
	</div>
	
	<div class='conditions'>
    <p>{{nyc.weather['0'].main}}</p>
		<img alt='' 
    class='icon-large' 
    v-bind:src="'../../static/'+ nyc.weather['0'].main + '.svg'" > 
    
	</div>
  <div class='time'>
    <p class='time'>{{ calcTime(-4) }}</p>
	</div>
	<div class='temps'>
		<p class='temp'>{{nyc.main.temp.toFixed(0)}}&deg;</p>
		<div class='temp-high-low'>
			<p class='temp-min'>{{nyc.main.temp_min.toFixed(0)}}&deg;</p>
			<p class='temp-high'>{{nyc.main.temp_max.toFixed(0)}}&deg;</p>
		</div>
	</div>
		...
		<div class='x-scroller'>
			<div class='sun'>
				<div>
					...
					<p>Sunrise</p>
            <p class='time'>{{getTime(nyc.sys.sunrise, 0, 0)}}</p>
				</div>
        </div>
        <div class='sun'>
				<div>
				...
				<p>Sunset</p>
          <p class='time'>{{getTime(nyc.sys.sunset, 0, 0)}}</p>
				</div>
			</div>
			<div class='wind'>
				<div>
					<p><img alt='' class='icon-small' src='https://image.flaticon.com/icons/svg/136/136743.svg'></p>
          <p><b>{{getWindDirection(nyc.wind.deg)}}</b> Wind </p>
        	<p class='time'>{{nyc.wind.speed}} mph</p>
				</div>
			</div>
			<div class='humidity'>
				<div>
					...
					<p>Humidity</p>
          <p class='time'>{{nyc.main.humidity}} %</p>
				</div>
			</div>
      <div class='pressure'>
        	<div>
					...
					<p>Pressure</p>
          <p class='time'>{{nyc.main.pressure}} hpa</p>
				</div>
      </div>
		</div>
	</div>
